 ...
 * ============================================================ */
#define NUM_QUEUES            5
#define MIN_SERVICE           3      /* min checkout time : 3 real min  */
#define MAX_SERVICE           12     /* max checkout time : 12 real min  */
#define MIN_PATIENCE          5      /* min patience      : 5 real min   */
#define MAX_PATIENCE          20     /* max patience      : 20 real min  */
#define JOCKEY_INTERVAL       5      /* jockey check every 5 ticks       */
#define ARRIVAL_CHANCE        55     /* 55% chance per tick of arrival   */
 ...
 * The condition  queues[shortest].size < queues[i].size - 1  ensures
 * the destination is at least 2 shorter (avoids pointless single-step moves).
 */
void phase_jockeying(Queue queues[]) {
    ...
    for (int i = 0; i < NUM_QUEUES; i++) {
        ...
        Customer* prev    = queues[i].front;
        Customer* current = queues[i].front->next;   /* only waiting customers */
        ...
        while (current != NULL) {
            ...
            if (shortest != i && queues[shortest].size < queues[i].size - 1) {
                sprintf(buf,
                    "C#%03d JOCKEYED -- Line %d -> Line %d (%d fewer people)",
                    current->id, i + 1, shortest + 1,
                    queues[i].size - queues[shortest].size);
                add_log(buf, CLR_MAGENTA);

                /* splice out of current line */
                prev->next = current->next;
                if (current == queues[i].rear) queues[i].rear = prev;
                queues[i].size--;

                /* enqueue into shorter line */
                Customer* sw = current;
                current = current->next;
                sw->next = NULL;
...
void show_final_report() {
    ...
    print_at(8, 16, CLR_CYAN, "--------------------------------------------------");

    if (total_served > 0) {
        float avg_wait = (float)total_wait / total_served;
        gotoxy(8, 17); set_colour(CLR_MAGENTA);
        ...
        gotoxy(8, 17); set_colour(CLR_DARK_GREY);
        printf("  No customers were served.");
    }

    print_at(8, 19, CLR_CYAN, "--------------------------------------------------");

    /* Efficiency summary */
    gotoxy(8, 20); set_colour(CLR_BRIGHT_WHITE);
    ...
    print_at(8, 22, CLR_CYAN, "--------------------------------------------------");
    print_at(8, 23, CLR_WHITE, "  Press ENTER to exit...");
    reset_colour();
    gotoxy(0, 25);
    getchar();
}
